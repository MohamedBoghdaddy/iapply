...
const EmployeeList = () => {
  ...
  const editEmployee = async (e) => {
    e.preventDefault();
    await axios
      .put(`http://localhost:8000/api/update/${editEmployeeId}`, employee)
      .then((response) => {
        toast.success(response.data.msg, { position: "top-right" });
        setView("list");
        fetchEmployees();
  ...
  const deleteEmployee = async (employeeId) => {
    await axios
      .delete(`http://localhost:8000/api/delete/${employeeId}`)
      .then((response) => {
  ...
  const startEditEmployee = async (employeeId) => {
    const response = await axios.get(
      `http://localhost:8000/api/getone/${employeeId}`
    );
  ...
  const renderEmployeeList = () => (
    <div className="employeetable">
      <button className="addbutton" onClick={() => setView("add")}>
        {" "}
        Add Employee
      </button>
          ...
          {employeeList.map((employee, index) => (
            <tr key={employee._id}>
              ...
              <td className="action">
                ...
                <button onClick={() => startEditEmployee(employee._id)}>
